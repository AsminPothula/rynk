...
async function findExistingRedirect(
  ...
  sourcePath: string,
): Promise<RedirectListItem | null> {
  const res = await client.request<RedirectListResponse>(
    "GET",
...
export async function applyAddRedirect(
  ...
  if (existing) {
    const updated = await client.request("PUT", `/redirection/v1/redirect/${existing.id}`, body);
  ...
  const created = await client.request("POST", "/redirection/v1/redirect", body);
